...
import {
  ...
  validateAccessToken,
} from "@central/auth/store";
import { getClient, registerClient, verifyClient } from "@central/db/client";
import { getHost } from "@central/db/host";
import { env } from "@central/env";
import { getHostRelayUrl } from "@central/relays/registry";
import {
  BadRequestError,
  ConflictError,
  ForbiddenError,
...
router.post("/ws-app-token", async (req, res) => {
  const user = requireAuthUser(req);
  ...
  // Steer the app to the SAME relay its host CLI is live on
  const relayUrl = getHostRelayUrl(clientInfo.hostId);
  if (!relayUrl) {
    throw new BadRequestError("Host is not available.");
  }

  // Persist the client device here (central owns the DB; the relay does not).
  registerClient(clientInfo);

  // convert the scheme to ws/wss for the websocket connection & add path for the app to connect
  const relayUrlObj = new URL(relayUrl);
  relayUrlObj.pathname = "/app";
  if (relayUrlObj.protocol === "http:") {
    relayUrlObj.protocol = "ws:";
  } else if (relayUrlObj.protocol === "https:") {
    relayUrlObj.protocol = "wss:";
  }

  const { token, ttlSeconds } = await createAppWebSocketToken(clientInfo);
