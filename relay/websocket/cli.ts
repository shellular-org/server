 ...
 * same relay. The CLI's identity is already proven by the signed host token at the
 * WebSocket upgrade (see `relay/upgrade.ts`), so there is no auth in here.
 */
...
function handleAuth(ws: WebSocket, msg: SessionHostMsg): Session {
  ...
  // No auth here: the host token was already verified at the WebSocket upgrade
  // (relay/upgrade.ts) and central signed it only after verifying the host against
  // the DB. The relay owns no DB, so it just builds the session from the message.
