 ...
 */
const analyticsEnabled = env.POSTHOG_KEY !== undefined && env.NODE_ENV === "prod";
logger.info(`🐽 PostHog analytics ${analyticsEnabled ? "enabled 📊" : "disabled"}`);

const client: PostHog | null =
  analyticsEnabled && env.POSTHOG_KEY ? new PostHog(env.POSTHOG_KEY, { host: env.POSTHOG_HOST }) : null;
...
export function captureAppConnection(userId: string, clientInfo: ClientInfo, hostPlatform: string): void {
  capture({
    distinctId: userId,
 ...
 */
export function captureLegacyAppConnection(clientInfo: ClientInfo, hostPlatform: string): void {
  capture({
