...
// Only real (prod) traffic should reach PostHog — dev runs would otherwise
// pollute DAU/retention with our own testing. Routing every event through this
// one gate means a newly added event can't forget to opt out of dev capture.
const analyticsEnabled = env.NODE_ENV === "prod";

function capture(event: Parameters<typeof posthog.capture>[0]): void {
	if (!analyticsEnabled) {
		return;
	}
