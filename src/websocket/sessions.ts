...
const socketToSession = new WeakMap<WebSocket, { session: Session; role: "host" | "client" }>();
const httpClientToSession = new Map<string, Session>();

export function createSession(machineId: string, host: WebSocket, hostInfo: Session["hostInfo"]): Session {
...
export function joinSession(machineId: string, client: WebSocket): Session | null {
...
export function joinSessionHttp(machineId: string, clientId: string): Session | null {
  const session = sessions.get(machineId);
  if (!session) return null;
...
export function getHttpClient(clientId: string): { session: Session; httpClient: HttpClient } | null {
...
export function rehostSession(machineId: string, newHost: WebSocket, hostInfo: Session["hostInfo"]): Session | null {
  const session = sessions.get(machineId);
  if (!session) return null;
...
export function appendTerminalBuffer(session: Session, terminalId: string, data: string): void {
...
export function getAndClearTerminalBuffer(session: Session, terminalId: string): string {
...
export function removeTerminalBuffer(session: Session, terminalId: string): void {
  session.terminalBuffers.delete(terminalId);
}
